...
- **Done and merged to `main`:** Part 0 (scaffold), Part 1 (plain HTTP coordinator/worker, PR #1), Part 2 (gRPC transport, PR #2), Part 3 (Redpanda event log + continuous-tail-free replay-once-at-startup, PR #3).
- **In progress:** Branch 4 (`part-4-raft`) — Tasks 4.1–4.8 all done (see `docs/plans/part-4-raft.md` and `PROGRESS.md`'s Part 4 section for the full task list). 3-replica `hashicorp/raft` cluster for leader election only (job data still flows through Part 3's Redpanda log, now continuously tailed by every replica, not just replayed once); writes are leader-gated and forwarded, raft state persists to disk. Real measured failover: 30-trial benchmark, median 106ms, p99 136ms, all under the 500ms target. `go build`/`go vet`/`go test -race` and the integration suite both pass; a 10-trial full-process-restart stress test (always submitting through a follower) passed 10/10. Not yet merged — next action is to open the PR for `part-4-raft` → `main`.
